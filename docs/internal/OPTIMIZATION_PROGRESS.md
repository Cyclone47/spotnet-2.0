# Spotnet 3.0 — Optimization Progress

Resumable checklist for the overhaul plan. Ticked items note the files touched so work can
be picked up cold in a new session.

Full plan with rationale: https://claude.ai/code/artifact/fe3abd9d-f066-4cff-ac26-3833a039e264

**Build:** 0 errors. **Tests:** 69/69 under the x64 test host.
The database durability work has been checked against a real user database. Live news-server,
full-import, WebView2 desktop, and genuinely corrupt-database smoke tests remain owner-run gates.
...
## x64 conversion completed

- All solution projects target AMD64 with `Prefer32Bit=false`.
- WebView2 is the sole embedded browser engine; all legacy browser code and assets were removed.
- The old x86 media wrapper was replaced with LibVLCSharp 3.10.1 and
  VideoLAN.LibVLC.Windows 3.0.23.1.
- SQLite stages and selects its x64 native interop library.
- The built application and test assembly both report `ProcessorArchitecture=Amd64`.
